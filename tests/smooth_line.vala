private void register_smooth_line() {
    Test.add_func("/LiveChart/SmoothLine/draw#shouldnt_render_if_no_values", () => {
        //Given
...
public void register_experimental_smooth_line() {
    Test.add_func("/LiveChart/SmoothLineSerie/draw#shouldnt_render_if_no_values", () => {
        //Given
        Cairo.ImageSurface surface = new Cairo.ImageSurface(Cairo.Format.ARGB32, SURFACE_WIDTH, SURFACE_HEIGHT);
        Cairo.Context context = new Cairo.Context(surface);
        cairo_background(context);

        var line = new LiveChart.SmoothLineSerie("Test");
        line.line.color = Gdk.RGBA() {red = 1.0, green = 0.0, blue = 0.0, alpha = 1.0 };
