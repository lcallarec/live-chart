private void register_bar() {
    Test.add_func("/LiveChart/Bar/draw", () => {
        ...
        var top_colors = unique_int_colors_at(pixbuff)(0, 0, 0, 3);
        assert(top_colors.size == 1);
        assert(top_colors.contains(color_to_int(red)));

        var bootom_colors = unique_int_colors_at(pixbuff)(0, 4, 0, 9);
    ...
    Test.add_func("/LiveChart/Bar/draw#shouldnt_render_if_no_values", () => {
        ...
        if (pixbuff != null) {
            var colors = unique_int_colors_at(pixbuff)(0, 0, SURFACE_WIDTH - 1, SURFACE_HEIGHT - 1);
