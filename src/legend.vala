...
namespace LiveChart {
    ...
    public abstract class BaseLegend : Drawable, LegendAware, Object {
     ...
     public class Legend : BaseLegend {
        ...
        public override void draw(Bounds bounds, Context ctx, Geometry geometry) {
            ...
            series.foreach((serie) => {
                ...
                ctx.rectangle(boundaries.x.min + pos, boundaries.y.max + 22, Legend.COLOR_BLOCK_WIDTH, 10);
                ctx.fill();
                
                ctx.move_to(boundaries.x.min + pos + Legend.COLOR_BLOCK_WIDTH + 3, boundaries.y.max + 20 + Geometry.FONT_SIZE);
